...
export default function ModalTaskForm({task_id, name, description, status, date_updated, date_created, onTaskSaved, onModalClose}){
	...
	function handleFormSubmit(e){
		...
		try {
			...
			if(task_id > 0){
				...
				RestAPI().updateTask(formValues,task_id).then((res) => {	
					handleResponse(res);
				});
				
			}else{
					
				RestAPI().createTask(formValues).then((res) => {	
					handleResponse(res);
				});
			}
	...
	return <>
			<Modal show={show} onHide={handleModalClose}>
				<form method="post" onSubmit={(e) => handleFormSubmit(e)}>
					...
					<Modal.Body>
						<TaskFormInputs task_id={task_id} name={name} description={description} status={status} date_created={date_created} date_updated={date_updated}  />
